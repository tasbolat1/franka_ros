...
bool FrankaStateController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  //fname = "stop";
  //ros::Subscriber sub = controller_node_handle.subscribe("filename", 1000, &FrankaStateController::filenameCallback, this);
  //End Add by weicong
...
void FrankaStateController::publishFrankaStates(const ros::Time& time) {
  if (publisher_franka_states_.trylock()) {
    ...
    for (size_t i = 0; i < endeffector_zero_jacobian.size(); i++) {
      publisher_franka_states_.msg_.jacobian[i] = endeffector_zero_jacobian[i];
    }

    Eigen::VectorXf dq(7);
    int i=0;
    for(i=0; i<7; i++)
      dq[i] = robot_state_.dq[i];
  
    // // Zhang Heng edited

    float r11 = robot_state_.O_T_EE[0];
    float r12 = robot_state_.O_T_EE[4];
    float r13 = robot_state_.O_T_EE[8];
    float r21 = robot_state_.O_T_EE[1];
    float r22 = robot_state_.O_T_EE[5];
    float r23 = robot_state_.O_T_EE[9];
    float r31 = robot_state_.O_T_EE[2];
    float r32 = robot_state_.O_T_EE[6];
    float r33 = robot_state_.O_T_EE[10];
    ...
    publisher_franka_states_.msg_.quaternion[0] = q0;
    publisher_franka_states_.msg_.quaternion[1] = q1;
    publisher_franka_states_.msg_.quaternion[2] = q2;
    publisher_franka_states_.msg_.quaternion[3] = q3;

    // END OF ZHANG HENG EDITED

    // record jacobian
    Eigen::VectorXf J_vector(42);
    for(i=0; i<endeffector_zero_jacobian.size(); i++)
      J_vector[i] = endeffector_zero_jacobian[i];

    Eigen::Map<Eigen::MatrixXf> J(J_vector.data(),6,7);

    // calculate cartesian velocity
    Eigen::VectorXf dx = J*dq;
    for (size_t i = 0; i < dx.size(); i++) {
      publisher_franka_states_.msg_.cartesian_velocity[i] = dx[i];
    }
    //End Add by weicong
